...
<div class="weui_cells weui_cells_form">
  <div class="weui_cell">
    ...
    <div class="weui_cell_bd weui_cell_primary">
      <input class="weui_input id" type="text" id="pickUpId"  placeholder="请输入订单号" value="${pickUp.id}" readonly>
    </div>
  </div>
...
<br>
  ...
  <a href="javascript:;" class="weui_btn weui_btn_primary" index="5">申请退款</a>
     ...
     var index = $(this).attr("index");
      ...
      if(index != undefined){
        if(index == 5 || index == "5"){
          ...
          $.confirm("你确定要退款?", "确认退款?", function() {
            $.ajax({
              type: "post",
              url: "${pageContext.request.contextPath}/pay/refund",
              data: {"pickUpId":$("#pickUpId").val()},
              dataType: "json",
              success: function(data){
                if(data.code == 0 || data.code == "0"){
                  $.toast("纯文本", "申请退款成功！");
                  $("#orderStatus").val("申请退款成功");
                }else{
                  $.toast("纯文本", "申请退款失败!");
                  $("#orderStatus").val("申请退款失败!");
                }
          ...
          $.ajax({
            type: "post",
            url: "${pageContext.request.contextPath}/pickUp/updatePickUpOrderStatus",
            data: {"pickUpId":$("#pickUpId").val(),"status":index},
            dataType: "json",
            success: function(data){
              $.toast("纯文本", "更新订单状态成功！");
              if("0" == index){
